...
provisioner "remote-exec" {
  inline = [
    ...
    "sed -i \"s/DB_NAME=store/DB_NAME=$DATABASE_NAME/\" .env",
    "sed -i \"s|STORE_URL=http://localhost/php-inventory-management-system/|STORE_URL=http://${aws_elastic_beanstalk_environment.eb_env.endpoint_url}/php-inventory-management-system/|\" .env",
    "git add .env",
